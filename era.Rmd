...
true_ERA_est <- function(ip_in, reported_ERA_in) {
  ...
  sim_df <- era_sim %>% as.data.frame () %>% dplyr::filter(round(IP, 1) == round(ip_in))
  
  #perfectly matching rows
  exact_df <- sim_df %>% dplyr::filter(reported_era == reported_ERA_in)
  ...
  if (nrow(exact_df) >= 1000) {
    ...
    true_mean <- mean(exact_df[exact_df$diff_rank <= 1000, 'true_era'])
  }
